...
import { Address, isSome, Some } from "@solana/kit";
...
export const PropertyIndicator: React.FC<PropertyIndicatorProps> = ({
  ...
  const getIndicatorPosition = (pos: number) => {
    ...
    if (pos === 0) {
      ...
      return {
        left: `${left}%`,
        top: `${100 - cornerCenter + indicatorOffset}%`,
      };
    } else if (pos === 10) {
      ...
      return {
        left: `${100 - cornerCenter + indicatorOffset}%`,
        top: `${top}%`,
      };
    } else {
      // Default fallback
...
interface PropertyIndicatorsContainerProps {
  ...
  propertyBuildings: {
    [position: number]: { houses: number; hasHotel: boolean; hasFlag: boolean };
  };
  mortgagedProperties: number[];
}

export const PropertyIndicatorsContainer: React.FC<
  PropertyIndicatorsContainerProps
> = ({
  propertyOwnership,
  properties,
  players,
  ...
  return (
    ...
    >
      ...
      {properties.map((property) => {
        ...
        const owner = isSome(property.owner)
          ? players.find(
              (p) => p.id === (property.owner as Some<Address>).value
            )
          : null;

        const buildings = propertyBuildings[position] || {
          houses: 0,
          hasHotel: false,
          hasFlag: false,
        };
