...
import { fakePlayerA } from "@/lib/sdk/utils";
...
export default function HomePage() {
  ...
  const handleCreatePlatform = async () => {
    try {
      ...
      const newPlatformId = await getAddressFromPublicKey(
        signer.keyPair.publicKey
      );

      const { instruction } = await sdk.createPlatformIx({
        rpc,
        creator: playerA,
      ...
      localStorage.setItem("platformId", newPlatformId.toString());
      setPlatformId(newPlatformId);

      toast.success(
        `Platform created successfully! ID: ${formatAddress(newPlatformId)}`
      );
    } catch (error) {
