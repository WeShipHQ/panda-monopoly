...
interface GameState {
    ...
    gamePhase: 'waiting' | 'rolling' | 'moving' | 'property-action' | 'special-action' | 'finished';
...
export const useGameManager = () => {
    ...
    const getPropertyRent = (position: number, state?: typeof gameState): number => {
        const currentState = state || gameState;
        const baseRent: { [key: number]: number } = {
            1: 2, 2: 4, 4: 6, 5: 8, 7: 10, 8: 12,
            10: 14, 11: 16, 13: 18, 14: 18, 16: 22,
            17: 24, 19: 26, 20: 26, 22: 35, 23: 50
        };
        ...
        if (colorGroup) {
            ...
            const ownsFullGroup = groupPositions.every(pos =>
                currentState.propertyOwnership[pos] === owner
            );
    ...
    const ownsFullColorGroup = (playerId: number, position: number): boolean => {
        ...
        return groupPositions.every(pos =>
            gameState.propertyOwnership[pos] === playerId
        );
    };

    // Add money and log
    const addMoney = useCallback((playerId: number, amount: number, reason: string) => {
        setGameState(prevState => {
            ...
            const newLog = [...prevState.gameLog, `${player?.name} ${reason} $${amount}`];
    ...
    const showMessage = useCallback((text: string, type: 'info' | 'warning' | 'success' | 'error' = 'info', duration: number = 3000) => {
        setGameState(prevState => ({
            ...prevState,
    ...
    // Define nextTurn first before it's used in other functions
    ...
    const handleSpaceLanding = useCallback((playerId: number, position: number) => {
        const space = boardSpaces[position];
            ...
            switch (space.type) {
                case 'corner':
                    if (space.name === 'GO') {
                        ...
                        newState.gameLog = [...prevState.gameLog, `${player.name} went to Jail!`];
                        ...
                        newState.gameLog = [...prevState.gameLog, `${player.name} rests at Free Parking`];
                ...
                case 'property':
                    ...
                    if (!owner) {
                        ...
                        newState.players = newPlayers;
                        newState.gameLog = [...prevState.gameLog,
                        `${player.name} paid $${rent} rent to ${ownerPlayer?.name}`];

                        // Show rent payment message
                        showMessage(`${player.name} paid $${rent} rent to ${ownerPlayer?.name}`, 'info', 2000);
                ...
                default:
                    ...
                    if (space.name?.includes('Tax') || space.name?.includes('Income Tax')) {
                        ...
                        newState.gameLog = [...prevState.gameLog, `${player.name} paid $${taxAmount} in taxes`];

                        // Show tax message
                        showMessage(`${player.name} paid $${taxAmount} in taxes`, 'warning', 2000);
            ...
            setGameState(prevState => {
                const newPlayers = prevState.players.map(player => {
                    if (player.id === playerId) {
                        ...
                        console.log(`Player ${player.id} moving from ${boardSpaces[currentPosition]?.name} (${currentPosition}) to ${boardSpaces[nextPosition]?.name} (${nextPosition})`);

                        // Check if passing through GO (but not landing on it)
                        const passedGo = currentPosition > nextPosition && nextPosition !== 0;
                        ...
                        if (passedGo) {
                            ...
                            moneyBonus = 200;
                            logMessage = `${player.name} landed on GO and collected $200`;
                        }
                ...
                const player = newPlayers.find(p => p.id === playerId);
                const passedOrLandedOnGo = currentStep < steps && (
                    player?.position === 0 ||
                    (prevState.players.find(p => p.id === playerId)?.position || 0) > (player?.position || 0)
                );
                ...
                return {
                    ...
                    gameLog: passedOrLandedOnGo ?
                        [...prevState.gameLog, `${player?.name} ${player?.position === 0 ? 'landed on' : 'passed'} GO and collected $200`] :
                        prevState.gameLog
                };
            });
    ...
    // Buy property
    const buyProperty = useCallback((position: number) => {
            ...
            setGameState(prevState => {
                const newPlayers = prevState.players.map(player =>
                    player.id === currentPlayer.id
                        ? { ...player, money: player.money - price, properties: [...player.properties, position] }
                        : player
                );
                ...
                return {
                    ...prevState,
                    players: newPlayers,
                    propertyOwnership: { ...prevState.propertyOwnership, [position]: currentPlayer.id },
                    gameLog: [...prevState.gameLog, `${currentPlayer.name} bought ${boardSpaces[position]?.name} for $${price}`],
                    gamePhase: 'waiting',
                    currentAction: undefined
                };
            });
    ...
    const skipProperty = useCallback(() => {
        setGameState(prevState => ({
            ...
            gamePhase: 'waiting',
            currentAction: undefined,
            gameLog: [...prevState.gameLog, `${gameState.players[gameState.currentPlayerIndex].name} skipped buying property`]
        }));
    ...
    interface Card {
        type: 'move' | 'money' | 'jail' | 'get-out-jail' | 'repairs' | 'collect-from-players' | 'tax';
        message: string;
    ...
    const chanceCards: Card[] = [
        { type: 'move', position: 0, message: 'Advance to GO (Collect $200)', collectGo: true },
        { type: 'move', position: 24, message: 'Advance to Illinois Avenue', collectGo: false },
        { type: 'move', position: 11, message: 'Advance to St. Charles Place', collectGo: false },
        { type: 'move', position: 'nearest-utility', message: 'Advance to nearest Utility', collectGo: false },
        { type: 'move', position: 'nearest-railroad', message: 'Advance to nearest Railroad', collectGo: false },
        { type: 'money', amount: 50, message: 'Bank pays you dividend of $50' },
        { type: 'get-out-jail', message: 'Get Out of Jail Free' },
        { type: 'move', position: 'back-3', message: 'Go Back 3 Spaces', collectGo: false },
        { type: 'jail', message: 'Go to Jail – Go directly to Jail' },
        { type: 'repairs', housePrice: 25, hotelPrice: 100, message: 'Make general repairs on all your property' },
        { type: 'money', amount: -15, message: 'Speeding fine $15' },
        { type: 'move', position: 5, message: 'Take a trip to Reading Railroad', collectGo: false },
        { type: 'move', position: 39, message: 'Take a walk on the Boardwalk', collectGo: false },
        { type: 'tax', amount: -15, message: 'You have been elected Chairman of the Board – Pay each player $50' },
        { type: 'money', amount: 150, message: 'Your building loan matures – Collect $150' },
        { type: 'money', amount: 100, message: 'You have won a crossword competition – Collect $100' }
    ];

    // Community Chest Cards
    const communityChestCards: Card[] = [
        { type: 'move', position: 0, message: 'Advance to GO (Collect $200)', collectGo: true },
        { type: 'money', amount: 200, message: 'Bank error in your favor – Collect $200' },
        { type: 'money', amount: -50, message: 'Doctor\'s fee – Pay $50' },
        { type: 'money', amount: 50, message: 'From sale of stock you get $50' },
        { type: 'get-out-jail', message: 'Get Out of Jail Free' },
        { type: 'jail', message: 'Go to Jail – Go directly to jail' },
        { type: 'money', amount: 100, message: 'Holiday fund matures – Receive $100' },
        { type: 'money', amount: 20, message: 'Income tax refund – Collect $20' },
        { type: 'money', amount: 10, message: 'Life insurance matures – Collect $100' },
        ...
        { type: 'repairs', housePrice: 40, hotelPrice: 115, message: 'You are assessed for street repairs' },
        { type: 'money', amount: 10, message: 'You have won second prize in a beauty contest – Collect $10' },
        { type: 'money', amount: 100, message: 'You inherit $100' },
        { type: 'collect-from-players', amount: 10, message: 'It is your birthday – Collect $10 from every player' }
    ];
    ...
    const handleSpecialCard = useCallback((cardType: 'chance' | 'community-chest') => {
        const cards = cardType === 'chance' ? chanceCards : communityChestCards;
        const currentIndex = cardType === 'chance' ? drawnCards.chanceIndex : drawnCards.communityChestIndex;
        ...
        setDrawnCards(prev => ({
            ...prev,
            [cardType === 'chance' ? 'chanceIndex' : 'communityChestIndex']: (currentIndex + 1) % cards.length
        }));
            ...
            switch (card.type) {
                ...
                case 'move':
                    ...
                    if (card.position === 'back-3') {
                        ...
                        newPosition = utilities.find(pos => pos > currentPlayer.position) || utilities[0];
                        ...
                        newPosition = railroads.find(pos => pos > currentPlayer.position) || railroads[0];
                        passedGo = newPosition <= currentPlayer.position;
                    } else {
                        newPosition = typeof card.position === 'number' ? card.position : 0;
                        passedGo = newPosition < currentPlayer.position && newPosition !== 0;
                    }
                    ...
                    newState.players = prevState.players.map(player =>
                        player.id === currentPlayer.id
                            ? { ...player, position: newPosition, money: player.money + bonusMoney }
                            : player
                    );
                    break;
                ...
                case 'repairs':
                    const houses = Object.values(prevState.propertyBuildings).reduce((total, building) =>
                        total + (building.houses || 0), 0
                    );
                    const hotels = Object.values(prevState.propertyBuildings).reduce((total, building) =>
                        total + (building.hasHotel ? 1 : 0), 0
                    );
                    const repairCost = houses * (card.housePrice || 0) + hotels * (card.hotelPrice || 0);
                ...
                case 'collect-from-players':
                    const totalCollected = (prevState.players.length - 1) * (card.amount || 0);
                    newState.players = prevState.players.map(player => {
                ...
                case 'tax':
                    const totalPaid = (prevState.players.length - 1) * Math.abs(card.amount || 0);
                    newState.players = prevState.players.map(player => {
                        if (player.id === currentPlayer.id) {
                            return { ...player, money: player.money - totalPaid };
                        } else {
                            return { ...player, money: player.money + Math.abs(card.amount || 0) };
                        }
                    });
        ...
        if (currentPlayer.money >= 50) {
            setGameState(prevState => ({
                ...
                players: prevState.players.map(player =>
                    player.id === currentPlayer.id
                        ? { ...player, money: player.money - 50, inJail: false, jailTurns: 0 }
                        : player
                ),
                gameLog: [...prevState.gameLog, `${currentPlayer.name} paid $50 to get out of jail`],
                gamePhase: 'waiting',
                currentAction: undefined
            }));
    ...
    const useJailFreeCard = useCallback(() => {
        ...
        if (playerJailCards > 0) {
            setGameState(prevState => ({
                ...
                gameLog: [...prevState.gameLog, `${currentPlayer.name} used Get Out of Jail Free card`],
                gamePhase: 'waiting',
                currentAction: undefined
            }));
    ...
    const tryJailDoubles = useCallback((dice1: number, dice2: number) => {
        const currentPlayer = gameState.players[gameState.currentPlayerIndex];
        ...
        setGameState(prevState => {
            ...
            if (isDoubles) {
                ...
                newState.gameLog = [...prevState.gameLog, `${currentPlayer.name} rolled doubles (${dice1}, ${dice2}) and got out of jail!`];
    ...
    // Build house
    const buildHouse = useCallback((position: number) => {
        const currentPlayer = gameState.players[gameState.currentPlayerIndex];
        const housePrice = 50; // Simplified house price

        if (currentPlayer.money >= housePrice && ownsFullColorGroup(currentPlayer.id, position)) {
            const currentBuildings = gameState.propertyBuildings[position] || { houses: 0, hasHotel: false };
            ...
            if (currentBuildings.houses < 4 && !currentBuildings.hasHotel) {
                ...
                if (colorGroup) {
                    const [_, groupPositions] = colorGroup;
                    const buildingCounts = groupPositions.map(pos =>
                        gameState.propertyBuildings[pos]?.houses || 0
                    );
                    ...
                    if (currentHouses < maxHouses + 1) {
                        setGameState(prevState => ({
                            ...
                            propertyBuildings: {
                                ...
                                [position]: { ...currentBuildings, houses: currentBuildings.houses + 1 }
                            },
                            gameLog: [...prevState.gameLog, `${currentPlayer.name} built a house on ${boardSpaces[position]?.name}`]
                        }));
                    }
                }
            }
        }
    }, [gameState.players, gameState.currentPlayerIndex, gameState.propertyBuildings, ownsFullColorGroup]);

    // Build hotel
    const buildHotel = useCallback((position: number) => {
        const currentPlayer = gameState.players[gameState.currentPlayerIndex];
        const hotelPrice = 50; // Simplified hotel price

        if (currentPlayer.money >= hotelPrice && ownsFullColorGroup(currentPlayer.id, position)) {
            const currentBuildings = gameState.propertyBuildings[position] || { houses: 0, hasHotel: false };
            ...
            if (currentBuildings.houses === 4 && !currentBuildings.hasHotel) {
                setGameState(prevState => ({
                    ...
                    gameLog: [...prevState.gameLog, `${currentPlayer.name} built a hotel on ${boardSpaces[position]?.name}`]
                }));
            }
        }
    }, [gameState.players, gameState.currentPlayerIndex, gameState.propertyBuildings, ownsFullColorGroup]);
    ...
    // Mortgage property
    const mortgageProperty = useCallback((position: number) => {
        const currentPlayer = gameState.players[gameState.currentPlayerIndex];
        const mortgageValue = Math.floor(getPropertyPrice(position) / 2);

        if (gameState.propertyOwnership[position] === currentPlayer.id &&
            ...
            if (!buildings?.houses && !buildings?.hasHotel) {
                setGameState(prevState => ({
                    ...
                    gameLog: [...prevState.gameLog, `${currentPlayer.name} mortgaged ${boardSpaces[position]?.name} for $${mortgageValue}`]
                }));
            }
        }
    }, [gameState.players, gameState.currentPlayerIndex, gameState.propertyOwnership, gameState.mortgagedProperties, gameState.propertyBuildings]);

    // Unmortgage property
    const unmortgageProperty = useCallback((position: number) => {
        const currentPlayer = gameState.players[gameState.currentPlayerIndex];
        ...
        if (gameState.propertyOwnership[position] === currentPlayer.id &&
            ...
            setGameState(prevState => ({
                ...
                mortgagedProperties: prevState.mortgagedProperties.filter(pos => pos !== position),
                gameLog: [...prevState.gameLog, `${currentPlayer.name} unmortgaged ${boardSpaces[position]?.name} for $${unmortgageValue}`]
            }));
        }
    }, [gameState.players, gameState.currentPlayerIndex, gameState.propertyOwnership, gameState.mortgagedProperties]);
    ...
    }, [gameState.players, gameState.currentPlayerIndex, movePlayer, setGamePhase, tryJailDoubles]);
