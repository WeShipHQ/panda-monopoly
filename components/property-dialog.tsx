...
export const PropertyDialog: React.FC<PropertyDialogProps> = ({
    ...
    return (
        <div className={`fixed inset-0 flex items-center justify-center z-50 transition-all duration-300 ${isOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'
            ...
            <div className="absolute inset-0 bg-white bg-opacity-40" onClick={onClose} />

            {/* Dialog with slide-up animation */}
            <div className={`relative bg-white border-2 border-gray-300 p-6 max-w-sm w-full mx-4 transition-all duration-300 ${isOpen ? 'translate-y-0 scale-100' : 'translate-y-8 scale-95'
                }`}>
                {/* Close button */}
                {onClose && (
                    <button
                ...
                <div className="flex gap-2">
                    ...
                    <button
                        onClick={onBuy}
                        disabled={!canAfford}
                        className={`flex-1 px-3 py-2 transition-colors text-sm ${canAfford
